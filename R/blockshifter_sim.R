...
## prepare a canned version of the correct objects so don't need to compute each time
## not run !
get_canned_contact_file<-function(){
    ## for the simulation we compare a test group CD4act + CD4nact against a control group mk and ery
    library(data.table)
    ...
    chic.thresh=5
    for(n in ti){
        cf[[n]]<-cf[[n]]>chic.thresh
    }
    ## only pick those PIRs where there is an interaction
    cf<-cf[which(rowSums(cf[,ti,with=FALSE])>0),]
    ## simulations are for chr1 only
    cf<-subset(cf, oeChr==1)
    setkey(cf,oeID)
    ...
    ## if gwas.gr is a file of all SNPs that will be simulated for chr1
}
...
blockshifter <-
  function(det.gr,gwas.gr,perm.no = 1e4,super.target.only = FALSE,target.only = TRUE) {
  test.set<-c('Total_CD4_Activated','Total_CD4_NonActivated')
  control.set<-c('Megakaryocytes','Erythroblasts')
  #message(paste(paste(test.set,sep='+'),"vs",paste(control.set,sep='+')))
  ...
  nomix.perms<-lapply(1:perm.no,function(i){
  ...
  pindex<-do.call("rbind",lapply(pindex[,2],function(y) sample(0:y,perm.no,replace=TRUE))) + pindex[,1]
  mix.perms<-lapply(seq(ncol(pindex)),function(i){
  ...
  output.df<-data.frame(type='ppi',gwas=gwas_trait,test='CD4A/N',control='MK/ERY',perm=perm.no,p.emp=pval.emp,p.emp.twotail=pval.emp.twotail,z=z,p.val.z=pval.z,delta=delta,var.null.delta=var(null.delta),mean.null.delta=mean(null.delta))
  output.df
}
...
option_list = list(
    make_option(c("-p","--perm_file"),type="character", default = NULL, help = "data.table file"),
    make_option(c("-o","--out_dir"),type="character", default = NULL, help = "out dir")
)

opt_parser = OptionParser(option_list = option_list)
opt = parse_args(opt_parser)

if(is.null(opt$perm_file)){
    print_help(opt_parser)
    stop("At least one argument must be supplied (perm file)",call.=FALSE)
}

if(is.null(opt$out_dir)){
    print_help(opt_parser)
    stop("Need and out_dir",call.=FALSE)
}
...
print(paste("Written",output.file))
